...
/**
 * A post-commit hook that performs inline minor log compaction.
 * It merges commit JSON files over a compaction interval into a single compacted JSON file.
 */
public class LogCompactionHook implements PostCommitHook {
  ...
  public LogCompactionHook(long startVersion, long commitVersion, Path logPath) {
  ...
  public void threadSafeInvoke(Engine engine) throws IOException {
    LogCompactionWriter compactionWriter =
        new LogCompactionWriter(logPath, startVersion, commitVersion);
    compactionWriter.writeMinorCompactionLog(engine);
  }
